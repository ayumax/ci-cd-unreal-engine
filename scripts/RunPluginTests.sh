...
# Get plugin name from environment variable or use first argument
# Default to "ObjectDeliverer" if not specified
PLUGIN_NAME="${PLUGIN_NAME:-${1:-ObjectDeliverer}}"

# Get project name from environment variable
# Default to plugin name + "Test" if not specified
PROJECT_NAME="${PROJECT_NAME:-${2:-${PLUGIN_NAME}Test}}"

echo "Starting $PLUGIN_NAME plugin tests with UE5..."
echo "Using project: $PROJECT_NAME"

# Specify the uproject file path using PROJECT_NAME
PROJECT_FILE="$PROJECT_DIR/$PROJECT_NAME.uproject"
PLUGIN_FILE="$PROJECT_DIR/Plugins/$PLUGIN_NAME/$PLUGIN_NAME.uplugin"

if [ ! -f "$PROJECT_FILE" ]; then
    echo "ERROR: Could not find $PROJECT_NAME.uproject at: $PROJECT_FILE"
    ls -la "$PROJECT_DIR"
    exit 1
fi
...
"$UE_ENGINE_DIR/Engine/Build/BatchFiles/Linux/Build.sh" ${PROJECT_NAME}Editor Linux Development -Project="$PROJECT_FILE" -TargetType=Editor
...
echo "Running $PLUGIN_NAME plugin tests..."
